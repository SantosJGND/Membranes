...
def frame_peaks(array_spec,
    ...
    if filter_keep:
        ...
        if filter_keep == "maxima":
            if len(cluster_centers) == 1:
                continue
            clust_dict= {
                amps_centres[x]: cluster_centers[x] for x in range(len(cluster_centers))
            }

            amps_sort=  sorted(clust_dict,key= clust_dict.get, reverse= True)
            cluster_centers= amps_sort[:2]
            amps_centres= [clust_dict[x] for x in cluster_centers]
